...
type AppState = Awaited<ReturnType<typeof resolveAppState>>;

function readModelInput(state: AppState) {
  return {
    scope: state.scope,
    displayCurrency: state.currency,
    period: state.period,
...
function withReadModel<T>(
  state: AppState,
  builder: (
    dataset: DomainDataset,
    input: ReturnType<typeof readModelInput>,
  ) => T,
) {
  return {
    ...state,
    ...builder(state.dataset, readModelInput(state)),
  };
}
...
export async function getSpendingModel(searchParams: RawSearchParams) {
  const state = await resolveAppState(searchParams);
  return withReadModel(state, buildSpendingReadModel);
}
...
export async function getIncomeModel(searchParams: RawSearchParams) {
  const state = await resolveAppState(searchParams);
  return withReadModel(state, buildIncomeReadModel);
}
