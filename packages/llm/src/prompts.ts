    ...
    editableSections: [
      {
        ...
        defaultValue: [
          ...
          "Once an exact ISIN is known for a mutual fund or index fund, lock identity from issuer-originated pages that explicitly reference that ISIN, then search dated fund-price pages keyed to that same ISIN and currency. If the issuer page confirms identity but does not expose the needed dated value, reputable fund-history pages such as Financial Times fund tearsheets on markets.ft.com/data/funds/tearsheet/historical are acceptable pricing sources only when they match the same ISIN and share-class currency.",
          ...
          "For mutual funds and non-exchange-traded index funds, use an explicit two-step workflow: first resolve identity, then retrieve NAV. When an exact ISIN is present, treat it as definitive for identity resolution and query issuer-originated pages, factsheets, product pages, KIIDs/KIDs, and regulator-grade listings that explicitly mention that ISIN to lock the exact fund and share class. Only after identity is exact should you perform a second search using the same ISIN together with terms like NAV, price, valuation, and fund value to retrieve the latest published NAV tied explicitly to that ISIN. Accept a NAV only when the dated source, currency, and fund structure are internally consistent with the resolved share class. For these instruments, prefer NAV over market quotes and do not stop after resolving the fund name if the NAV search has not been attempted.",
