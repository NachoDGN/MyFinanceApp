...
type FlowCategoryAmountRow = {
  categoryCode: string;
  label: string;
  amountEur: string;
};

type SpendingCategoryAmountRow = FlowCategoryAmountRow;
...
type IncomeCategoryAmountRow = FlowCategoryAmountRow;
...
function serializeCategoryAmountRows(
  rows: Iterable<{
    categoryCode: string;
    label: string;
    amountEur: Decimal;
...
function aggregateFlowCategoryRows(
  ...
  bucketFor: (
    categoryByCode: Map<string, DomainDataset["categories"][number]>,
    transaction: Transaction,
  ) => { categoryCode: string; label: string },
) {
  const totals = transactions.reduce((groups, transaction) => {
    const contribution = contributionFor(transaction);
    if (!contribution) {
      return groups;
    }

    const bucket = bucketFor(categoryByCode, transaction);
    const current = groups.get(bucket.categoryCode) ?? {
    ...
    groups.set(bucket.categoryCode, current);
    return groups;
  }, new Map<string, { categoryCode: string; label: string; amountEur: Decimal }>());

  return serializeCategoryAmountRows(totals.values());
}
...
function aggregateSpendingCategoryRows(
  ...
  return aggregateFlowCategoryRows(
    transactions,
    categoryByCode,
    spendingContributionEur,
...
function buildMonthlyCategorySeries<TTotalKey extends string>(
  dataset: DomainDataset,
  scope: Scope,
  startDate: string,
  endDate: string,
  ...
  {
    month: string;
    categories: FlowCategoryAmountRow[];
  } & Record<TTotalKey, string>
> {
  const seriesStart = startOfMonthIso(startDate);
  const seriesEnd = startOfMonthIso(endDate);
  ...
  for (const transaction of flowTransactionsInWindow(
    ...
    row.totalEur = row.totalEur.plus(contribution);
    row.categoryTotals.set(bucket.categoryCode, category);
  }

  return [...monthRows.values()].map((row) => ({
    month: row.month,
    [totalKey]: row.totalEur.toFixed(2),
    categories: serializeCategoryAmountRows(row.categoryTotals.values()),
  })) as Array<
    {
      month: string;
      categories: FlowCategoryAmountRow[];
...
function buildMonthlySpendingCategorySeries(
  ...
  return buildMonthlyCategorySeries(
    dataset,
    scope,
    startDate,
    endDate,
...
function isFlowCategoryTransaction(
  categoryByCode: Map<string, DomainDataset["categories"][number]>,
  categoryCode: string,
  transaction: Transaction,
  contributionFor: (transaction: Transaction) => Decimal | null,
  ...
  return Boolean(contributionFor(transaction)) &&
    bucketFor(categoryByCode, transaction).categoryCode === categoryCode;
}

function flowCategoryAmountForPeriod(
  dataset: DomainDataset,
  scope: Scope,
  period: PeriodSelection,
  categoryByCode: Map<string, DomainDataset["categories"][number]>,
  ...
  return filterTransactionsByPeriod(
    ...
    .reduce((sum, transaction) => {
      ...
      return sum.plus(contributionFor(transaction) ?? new Decimal(0));
    }, new Decimal(0));
}

function categoryTransactionSummary(
  transactions: Transaction[],
...
function categoryComparison(
  ...
  return {
    periodSharePercent: totalAmount.gt(0)
      ? categoryAmount.div(totalAmount).mul(100).toFixed(2)
      : null,
    comparisonDeltaPercent: comparisonAmount.eq(0)
      ? null
      : categoryAmount
